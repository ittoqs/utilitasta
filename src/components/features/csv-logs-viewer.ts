...
// Nama kolom berkardinalitas tinggi yang tidak boleh menjadi faset
...
// Kolom yang baik untuk faset dalam log
...
export function buildSmartFacets(
  ...
  // Pertama, buat semua faset untuk menganalisis kardinalitas
  const facetMaps = new Map<string, Map<string, number>>();
  headers.forEach((header) => facetMaps.set(header, new Map()));
  ...
  for (const header of headers) {
    ...
    // Tentukan apakah kolom ini harus menjadi faset
    let shouldInclude: boolean;
    // Selalu sertakan kolom yang ramah faset jika mereka memiliki kardinalitas yang wajar
    if (isFacetFriendlyColumn(header)) {
      shouldInclude = uniqueCount <= 100 && uniqueCount > 1;
    }
    // Kecualikan kolom pola berkardinalitas tinggi
    else if (isHighCardinalityColumn(header)) {
      shouldInclude = false;
    }
    // Untuk kolom lain, gunakan heuristik cerdas
    else {
      // Sertakan jika: rasio kardinalitas rendah DAN jumlah unik wajar
      // - Kurang dari 30% nilai unik (baik untuk pengelompokan)
      // - Atau kurang dari 50 total nilai unik
      // - Dan lebih dari 1 nilai unik (jika tidak, tidak ada gunanya)
      shouldInclude =
        uniqueCount > 1 &&
